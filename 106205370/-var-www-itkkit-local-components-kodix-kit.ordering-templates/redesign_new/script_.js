...
$(function(){
	...
	$(document).on('change', 'select[name=PROFILE]', function(){
		...
		$.ajax({
			...
			let arCurrentClassFlag = $(".header-action__dd-for > .icon-flag").attr("class").split(" ");
			...
			$(".header-action__dd-for > .icon-flag").removeClass(currentClassName).addClass(newClassName);
			$(".header-action__dd-for > .icon-flag").next('span').html(data);
			console.log(data);
		})
