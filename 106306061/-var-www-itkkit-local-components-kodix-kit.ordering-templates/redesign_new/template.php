...
//    echo '<pre>';
//    var_dump ($arResult['ORDER']->calculatedOrder);
//    echo '</pre>';
//    echo '<pre>';
//    print_r ($_SESSION['LAST_COUNTRY']);
//    echo '</pre>';
    ...
    foreach($arResult["AVAILABLE"] as $arItem){
        if ($useVAT=="Y") { //кейс, есть VAT Цены укаываем С НДС
            ...
            $item_array['VAT']=$item_array['PRICE_QUANTITY']-round($item_array['PRICE_QUANTITY'] / 1.21,2);
        } else { //кейс, когда нету VAT цены считаем БЕЗ НДС
            $item_array['PRICE_QUANTITY']=round($arItem['PRICE']/1.21*$arItem['QUANTITY'],2);
            $item_array['OLD_PRICE_QUANTITY']=round(($arItem['PROPS']['BASE_PRICE_P'] / 1.21)*$arItem['QUANTITY'],2);
            $item_array['VAT']=0;
        }
                                                ...
                                                <td>
                                                    <div class="price__wrapper vat">
                                                        <div class="price">
                                                            <span class="price__block">
                                                                <span class="price--current">
                                                                    <?=KDXCurrency::convertAndFormat($total_array['TOTAL']['VAT'],KDXCurrency::$CurrentCurrency);?>
                                                                </span>
                                                            </span>
                                                        </div>
                                                    </div>
