...
class KDXDeliveryEMS{

    function Init(){
        if(defined('ADMIN_SECTION') && ADMIN_SECTION && $_SERVER['SCRIPT_URL']=='/bitrix/admin/sale_delivery_handler_edit.php' && $_GET['SID']=='kdx_ems'){
        ...
        return Array(
            ...
            'SORT'              => 30,
            'DESCRIPTION'       => '',
            'DESCRIPTION_INNER' => 'Доставка EMS',
            'BASE_CURRENCY'     => COption::GetOptionString('sale', 'default_currency', 'RUR'),

            'HANDLER'           => __FILE__,

            /* Методы обработчика */
            'DBGETSETTINGS'     => array('KDXDeliveryEMS', 'GetSettings'),
            'DBSETSETTINGS'     => array('KDXDeliveryEMS', 'SetSettings'),
            'GETCONFIG'         => array('KDXDeliveryEMS', 'GetConfig'),

            'COMPABILITY'       => array('KDXDeliveryEMS', 'Compability'),
            'CALCULATOR'        => array('KDXDeliveryEMS', 'Calculate'),
            ...
            'PROFILES' => array(
                'ems' => array(
                    ...
                    'RESTRICTIONS_WEIGHT' => array(0,31500), // без ограничений
                    'RESTRICTIONS_SUM'    => array(0), // без ограничений
                    'RESTRICTIONS_DIMENSIONS' => array(1500,1500,1500),
                )
            )
        );
    }
        ...
        $arConfig = array(
            ...
            "CONFIG" => array(
                ...
                'DISCOUNT_DELIVERY'=>array(
                    "TYPE" => "STRING",
                    "DEFAULT" => "50",
                    "TITLE" => 'Возможная скидка на доставку(% или '.COption::GetOptionString('sale', 'default_currency', 'RUR').')',
                    "GROUP" => "price",
                ),
...
//            die();
        ...
        $itemsCountInSections = CalculateBasketItems();

        if ($itemsCountInSections["FOOTWEAR"] >= 2) {
            $deliveryOverPrice = 10 * ($itemsCountInSections["FOOTWEAR"] - 1);
            $price = $price + $deliveryOverPrice;
            if (!isset($_SESSION['SHOW_DELIVERY_MESSAGE'])) $_SESSION['SHOW_DELIVERY_MESSAGE'] = true;
        }

        if ($itemsCountInSections["DECKS"] > 0) {
            $deliveryOverPrice = 10 * $itemsCountInSections["DECKS"];
            $price = $price + $deliveryOverPrice;
            if (!isset($_SESSION['SHOW_DELIVERY_DECKS_MESSAGE'])) $_SESSION['SHOW_DELIVERY_DECKS_MESSAGE'] = true;
        }
