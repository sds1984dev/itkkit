    ...
    <script>
        gtag('get', 'UA-52518243-2', 'client_id', (id) => {
            ...
            let data = 
                ...
                + "&cid=" + encodeURI(CID)
                + "&t=" + encodeURI('pageview')
                + "&dh=" + encodeURI('itkkit.com')
                + "&dp=" + encodeURI('/payment_success')
                + "&dt=" + encodeURI('Заказ оформлен')
                + "&ti=" + encodeURI('<?=$arResult['ORDER']->id?>')
                + "&tr=" + encodeURI('<?=$arResult['ORDER']->price?>')
                + "&ts=" + encodeURI('<?=$arResult['ORDER']->price_delivery?>')
            ;  
            <?$i = 0;?>
                        ...
                        ?>
                        data = data 
                        + "&pa=" + encodeURI('purchase')
                        + "&pr<?=$i?>id=" + encodeURI('<?=$item['PROPS']['PARENT_ID']?>')
                        + "&pr<?=$i?>nm=" + encodeURI('<?=$newName?>')
                        + "&pr<?=$i?>ca=" + encodeURI('<?=$path?>')
                        + "&pr<?=$i?>br=" + encodeURI('<?=$brands?>')
                        + "&pr<?=$i?>pr=" + encodeURI('<?=$item['PRICE']?>')
                        + "&pr<?=$i?>qt=" + encodeURI('<?=$item['QUANTITY']?>');
                    <?endforeach;?>
                    <?unset($i);?>
            console.log(data); 
            $.post(
