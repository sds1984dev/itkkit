...
if(!defined("B_PROLOG_INCLUDED") || B_PROLOG_INCLUDED!==true) die();
    ...
    $APPLICATION->addChainItem((SITE_ID == 's1' ? "Бренды" : "Brands"), "/".$arCurPage[2]);
    $dbBrand = CIBlockElement::GetList(array(), array("CODE" => $arCurPage[3]))->Fetch();
    $APPLICATION->addChainItem($dbBrand['NAME'], "/catalog/".$arCurPage[2]."/".$arCurPage[3]);

    $dbSection = CIBlockElement::GetList(array(), array("IBLOCK_ID" => 3, "CODE" => $arCurPage[3]))->Fetch();
    $arResult['SELECTION_DATA']['DATA']['DETAIL_TEXT'] = $dbSection['DETAIL_TEXT'];
    $dbEnSection = CIBlockElement::GetProperty(3, $dbSection['ID'], array(), array("CODE" => "EN_DETAIL_TEXT"));
    while ($arProps = $dbEnSection->Fetch()) {
        $arResult['SELECTION_DATA']['DATA']['PROPERTY_DETAIL_TEXT_EN_VALUE'] = $arProps['VALUE'];
    }
...
<?}?>
<?if(!isAjax()){?>
    <div class="catalog-section__tags" style="margin-bottom: 40px;">
        <?
        ...
        while ($arTag = $resTag->Fetch()){
            echo '<a href="'.$getCurUrl.'tag/'.$arTag['CODE'].'/">[ '.strtoupper($arTag['NAME']).' ]</a>';
        }
...
<!--                    <h1 class="heading--h1">--><?//=GetMessage('MORE_PRODUCTS_COMING_SOON')?><!--</h1>-->
                    <h2 class="heading--h1"><?=GetMessage('TRY_CHANGE')?></h2>
