...
class CBrandsListComponent extends CBitrixComponent
{
    public function executeComponent()
    {
        ...
        $full_alphabet = array_merge(range('0', '9'),range('A', 'Z'), range('a', 'z'), $russian, $russian2 ); // range('0', '9')
        $uppercase_alphabet = array_merge(range('0', '9'),range('A', 'Z'),$russian, array('#')); // ,
		...
		foreach ($arBrands as $brend) {
			if (isset($arAvailableBrands[$brend['ID']])) {
				...
				$arAvailableBrands[$brend['ID']]['available']=0;*/
			}
		}
        ...
        foreach($arAvailableBrands as $item) {
            ...
            if (in_array($alpha,$full_alphabet))
                ...
                $brands['#'][] = array('DETAIL_PAGE_URL' => $item['DETAIL_PAGE_URL'], 'NAME' => $item['NAME']);
        }
