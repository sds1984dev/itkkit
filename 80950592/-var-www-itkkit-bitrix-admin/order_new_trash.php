		...
		foreach ($arBasketItem as $val)
		{
            ...
            $basePrice = 0;
               ...
               $dbBasketProps = CSaleBasket::GetPropsList(Array(), Array("BASKET_ID" => $dbBasket['ID']));
                ...
                {
                    if ($prop['CODE'] == 'BASE_PRICE_P')
                    {
                        $basePrice = $prop['VALUE'];
                        break;
                    }
                }
		...
		if (result.length > 0)
		{
			...
			for (i in res)
			{
				...
				if (i > 0)
				{
                    ...
                    let price = document.getElementById('PRODUCT[' + i + '][PRICE]').value;
                    console.log(parseFloat(BX('PRODUCT[' + i + '][PRICE_DEFAULT]').value));
                    //BX('PRODUCT[' + i + '][PRICE]').value = parseFloat(BX('PRODUCT[' + i + '][PRICE_DEFAULT]').value);
                    //BX('PRODUCT[' + i + '][PRICE]').value = parseFloat(price);


					//BX('formated_price_' + i).innerHTML = res[i]["PRICE_DISPLAY"];
                    BX('formated_price_' + i).innerHTML = parseFloat(price);
					...
					if (res[i]["DISCOUNT_REPCENT"] != 0)
					{
						if (BX('PRODUCT[' + i + '][CUSTOM_PRICE]').value != "Y")
						{
                            BX('formated_price_' + i).innerHTML = (parseFloat(price) - parseFloat(price) * res[i]["DISCOUNT_REPCENT"] / 100).toFixed(2);
                            ...
                            BX('DIV_BASE_PRICE_WITH_DISCOUNT_' + i).innerHTML = parseFloat(price);
                    ...
                    var deliveryDiff = (BX('PRICE_DELIVERY_DIFF').value).replace('€', '');

					//BX('DIV_SUMMA_' + i).innerHTML = "<div>" + res[i]["SUMMA_DISPLAY"] + " <span>"+res[0]["CURRENCY_FORMAT"]+"</span></div>";
                    var summa = (parseFloat(price) * parseFloat(res[i]["QUANTITY"])).toFixed(2);
                    total = total + parseFloat(summa);
					BX('DIV_SUMMA_' + i).innerHTML = "<div>" + summa + " <span>"+res[0]["CURRENCY_FORMAT"]+"</span></div>";

                    //BX('PRODUCT[' + i + '][PRICE_DEFAULT]').value = res[i]["PRICE"];
                    BX('PRODUCT[' + i + '][PRICE_DEFAULT]').value = parseFloat(price);
			...
			{
				BX('DELIVERY_PRICE_DIFF').innerHTML = res[0]["PRICE_DELIVERY_DIFF"] - deliveryDiff;

				BX('PRICE_DELIVERY_DIFF').value = res[0]["PRICE_DELIVERY_DIFF"] - parseFloat(deliveryDiff);
			...
			if (res[0]["ORDER_ERROR"] == "N")
			{
				...
				BX('ORDER_DELIVERY_PRICE').innerHTML = BX('DELIVERY_ID_PRICE').value - deliveryDiff;


				//BX('ORDER_TAX_PRICE').innerHTML = res[0]["PRICE_TAX"];
				BX('ORDER_WAIGHT').innerHTML = res[0]["PRICE_WEIGHT_FORMAT"];
				BX('ORDER_PRICE_ALL').innerHTML = parseFloat(total.toFixed(2)) + parseFloat(BX('DELIVERY_ID_PRICE').value) - parseFloat(deliveryDiff);//res[0]["PRICE_TO_PAY"];
