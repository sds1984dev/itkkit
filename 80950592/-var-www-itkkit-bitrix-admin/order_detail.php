...
else
{
	$arUserColumns = array(
        ...
        "COLUMN_BASE_PRICE" => 'Базовая цена'
	);
}
...
if (isset($_REQUEST["ORDER_AJAX"]) AND $_REQUEST["ORDER_AJAX"] == "Y" AND check_bitrix_sessid())
{
	...
	*/
	...
	/*
	 * save tracking weight
	 */
	if (isset($_REQUEST["tracking_weight"]) && strlen($_REQUEST["tracking_weight"]) >= 0)
	{
		$ID = IntVal($ID);
		$clear_value = preg_replace('/[^0-9]/', '', trim($tracking_weight));
		$tracking_weight = str_replace(array(' ', ','), array('', '.'), $tracking_weight);

		$bUserCanEditOrder = CSaleOrder::CanUserUpdateOrder($ID, $arUserGroups);

		if (isset($change) && $change == "Y" && $bUserCanEditOrder && !CSaleOrder::IsLocked($ID, $lockedBY, $dateLock))
		{
			CUtil::DecodeUriComponent($tracking_weight);
			CSaleOrder::Update($ID, array("TRACKING_WEIGHT" => $tracking_weight));
		}
		$arResult = array('message' => 'ok');
		$result = CUtil::PhpToJSObject($arResult);
...
$dbOrder = CSaleOrder::GetList(
	...
	array(
		...
		"DELIVERY_DOC_DATE", "STORE_ID", "ACCOUNT_NUMBER", "TRACKING_NUMBER", "TRACKING_WEIGHT"
	)
);
if (!($arOrder = $dbOrder->Fetch()))
	LocalRedirect("sale_order.php?lang=".LANGUAGE_ID.GetFilterParams("filter_", false));
		...
		$tabControl->BeginNextFormTab();
					...
					<tr>
						...
						<td width="60%">
									...
									<div id="editStatusDIV">
        								...
        								<script async src="https://www.googletagmanager.com/gtag/js?id=UA-52518243-2>"></script>
											...
											function fChangeStatusResult(res)
											{
												...
												if (rs.STATUS_ID == 'C') {
										            ...
										            gtag('get', 'UA-52518243-2', 'client_id', (id) => {
										                ...
										                let data = 
										                "v=" + encodeURI('1')
										                + "&tid=" + encodeURI('UA-52518243-2')
										                + "&cid=" + encodeURI(CID)
										                + "&t=" + encodeURI('event')
										                + "&ec=" + encodeURI('Ecommerce')
										                + "&ea=" + encodeURI('Refund')
										                + "&ni=" + encodeURI('1')
										                + "&ti=" + encodeURI('<?=$arOrder['ID']?>')
										                + "&pa=" + encodeURI('refund');
			...
			$tabControl->AddSection("order_user", GetMessage("P_ORDER_USER_ACC"));
				...
				?>
					...
					{
                        if ($arOrderProps['CODE'] == 'DELIVERY_COUNTRY') $deliveryCountryCode = $arOrderProps['VALUE'];
			...
			$tabControl->AddSection("order_delivery", GetMessage("P_ORDER_DELIVERY_TITLE"));
				...
				$tabControl->BeginCustomField("ORDER_TRACKING_WEIGHT", GetMessage("SOD_ORDER_TRACKING_WEIGHT"));
					?>
					<tr>
						<td width="40%"><?echo $tabControl->GetCustomLabelHTML()?>:</td>
						<td width="60%">
							...
							{
							?>
								<span id="tracking-weight-title" onclick="fChangeTrackingWeight(this);">
									<?
									if ('' != $arOrder["TRACKING_WEIGHT"])
										echo htmlspecialcharsbx($arOrder["TRACKING_WEIGHT"]);
									else
										echo GetMessage('SOD_ORDER_TRACKING_WEIGHT_ADD');
									?>
								</span>
							...
							else
							{
								?><span id="tracking-weight-title">
									...
									{
										echo '<span style="text-decoration: line-through;" title="'.$strLockUserExt.'">'.GetMessage('SOD_ORDER_TRACKING_WEIGHT_ADD').'</span>';
									}
									?>
								</span>
								<?
							...
							{
							?>
							<input type="text" id="tracking-weight-text" name="TRACKING_WEIGHT" onChange="fEditTrackingWeight(this, 'change');"
								onBlur="fEditTrackingWeight(this, 'exit');" value="<?=htmlspecialcharsbx($arOrder["TRACKING_WEIGHT"])?>" size="30">
							<?
							}
							else
							{
								?><input type="hidden" value="<?=htmlspecialcharsbx($arOrder["TRACKING_WEIGHT"])?>" name="TRACKING_WEIGHT" id="tracking-weight-text"><?
							}
							?>
							<input type="hidden" name="change_tracking_weight" id="id_change_tracking_weight_hidden" value="N">

							<script type="text/javascript">
								function fChangeTrackingWeight(el)
								{
									BX(el).style.display = 'none';
									BX('tracking-weight-text').style.display = 'block';
									BX('tracking-weight-text').focus();

								}

								function fEditTrackingWeight(el, type)
								{
									if (type == 'change')
									{
										BX.showWait();

										BX.ajax.post('/bitrix/admin/sale_order_detail.php', '<?=CUtil::JSEscape(bitrix_sessid_get())?>&ORDER_AJAX=Y&change=Y&tracking_weight='+BX.util.urlencode(el.value)+'&ID=<?=$ID?>', fEditTrackingWeightResult);

										if (BX('tracking-weight-text').value.length > 0)
											BX('tracking-weight-title').innerHTML = BX('tracking-weight-text').value;
										else
											BX('tracking-weight-title').innerHTML = '<?=GetMessage('SOD_ORDER_TRACKING_WEIGHT_ADD')?>';
									}

									BX('tracking-weight-title').style.display = 'inline-block';
									BX('tracking-weight-text').style.display = 'none';
								}

								function fEditTrackingWeightResult(res)
								{
									BX.closeWait();
								}
							</script>
				...
				<?
				$tabControl->EndCustomField("ORDER_TRACKING_WEIGHT", "");
				...
				<tr>
					<td colspan="2" valign="top">
						<!-- ////////////////////////////////// START CUSTOM CODE ////////////////////////////////// -->
						<link rel="stylesheet" type="text/css" href="/local/templates/kit/js/jquery.fancybox.min.css"/>
						<script type="text/javascript" src="/local/templates/kit/js/jquery-1.11.1.min.js"></script>
						<script type="text/javascript" src="/local/templates/kit/js/jquery.fancybox.min.js"></script>
						<!-- ////////////////////////////////// END CUSTOM CODE ////////////////////////////////// -->
						...
						if(!empty($arBasketId))
						{
                            ...
                            if (isset($hlblock['ID']))
                            {
                                ...
                                $res = $entity_data_class::getList( array('filter'=>array('UF_COUNTRY_ID'=>$deliveryCountryCode)));

                                while ($item = $res->fetch())
                                {
                                    if ((isset($item['UF_USE_VAT'])) && ($item['UF_USE_VAT']=='Y')) $useVAT = true;
                                }
                            }
...
//                                echo "</pre>";
                                ...
                                {
                                    if ($val['CODE'] == 'BASE_PRICE_P') $basePrice = $val['VALUE'];
                                }
...
//                                $arItem['VAT_RATE'] = ($group == 554) ? 1.12 : 1.21;
//                                $arItem["PRICE"] = number_format(($useVAT) ? $arItem["PRICE"] : $arItem["PRICE"] / $arItem["VAT_RATE"], 2, '.', '');

								if (!CSaleBasketHelper::isSetItem($arItem))
								{
									$ORDER_TOTAL_PRICE_WITH_DISCOUNT += $arItem["PRICE"] * $arItem["QUANTITY"];
								...
								foreach ($arUserColumns as $columnCode => $columnName)
								{
									...
									if ($columnCode == "COLUMN_IMAGE")
									{
										...
										<td class="COLUMN_IMAGE">
											...
											$arCustomProduct = [];
											$customResOffer = CCatalogSku::GetProductInfo($arItem['PRODUCT_ID']);
											if (is_array($customResOffer)){
												$customResProduct = CIBlockElement::GetList(array(), array('IBLOCK_ID'=>1, 'ID'=>$customResOffer['ID']), false, array(), array('ID', 'IBLOCK_ID', 'CODE', 'DETAIL_PICTURE'));
												if ($customArProduct = $customResProduct->Fetch()){
													$arCustomProduct = $customArProduct;
												}
											}
											if (!empty($arCustomProduct)){
												if (!empty($arCustomProduct['DETAIL_PICTURE'])){
													$customProductImg = CFile::ResizeImageGet($arCustomProduct['DETAIL_PICTURE'], array('width'=>80, 'height'=>80), BX_RESIZE_IMAGE_PROPORTIONAL, false, false);
													echo '<a href="'.CFile::GetPath($arCustomProduct['DETAIL_PICTURE']).'" data-fancybox="cartImg'.$arCustomProduct['ID'].'"><img src="'.$customProductImg["src"].'" width="80" height="80" alt="" border="0" /></a>';
												} else {
													echo '<div class="no_foto">'.GetMessage('SOD_NO_FOTO').'</div>';
												}
											} else {
												if (is_array($arItem["PICTURE"]))
													echo '<img src="'.$arItem["PICTURE"]["src"].'" width="80" height="80" alt="" border="0" />';
												else
													echo '<div class="no_foto">'.GetMessage('SOD_NO_FOTO').'</div>';
											}
											?>
									...
									if ($columnCode == "COLUMN_NAME")
									{
										...
										<td class="COLUMN_NAME">
											...
											if (!empty($arCustomProduct)){?>
												<a href="/catalog/product/<?=$arCustomProduct['ID']?>_<?=$arCustomProduct['CODE']?>/" class="name-link <?=$linkClass?>" target="_blank"><?=trim($arItem["NAME"]);?></a>
											<?} else {
									...
									if ($columnCode == "COLUMN_QUANTITY")
									{
										...
										<td class="COLUMN_QUANTITY">
											...
											if ($arItem["QUANTITY"] > 1){
												echo '<span style="font-weight: bold; color: red">'.$arItem["QUANTITY"]."&nbsp".$measure.'</span>';
											} else {
                                    ...
                                    if ($columnCode == "COLUMN_BASE_PRICE")
                                    {
                                        ?>
                                        <td class="COLUMN_BASE_PRICE" nowrap style="in-width: 100px !important; text-align: right !important; vertical-align: top;">
                                            <div class="edit_price">
													<span class="default_price_product" >
														<span class="formated_price"><?=CCurrencyLang::CurrencyFormat($basePrice, $arItem["CURRENCY"], false);?></span>
													</span>
                                                <span class="currency_price"><?=$CURRENCY_FORMAT?></span>
                                            </div>
                                            <?

                                            ?><div class="base_price_title" style= "font-size: 11px;color: #717171;">
                                                <?=strtoupper('base')?>
                                            </div>
                                        </td>
                                        <?
								...
								<div class="order-itog">
									<table>
										...
										<tr class="price">
											...
											<td nowrap style="white-space:nowrap;"><?=SaleFormatCurrency($ORDER_TOTAL_PRICE_WITH_DISCOUNT, $arOrder["CURRENCY"]);?></td>
										</tr>
										<tr>
										...
										<tr class="itog">
											...
											<td class="iright" nowrap><div style="white-space:nowrap;"><?=SaleFormatCurrency($ORDER_TOTAL_PRICE_WITH_DISCOUNT + $arOrder["PRICE_DELIVERY"], $arOrder["CURRENCY"]);?></div></td>
										</tr>
