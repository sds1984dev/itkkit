	...
	&& empty($_POST['dontsave']))
{
	...
	if (strlen($errorMessage) <= 0)
	{
        ...
        foreach ($arShoppingCart as $key => $item)
        {
            $arShoppingCart[$key]['DISCOUNT_PRICE'] = $_POST["PRODUCT"][$item['BASKET_ID']]['DISCOUNT_PRICE'];
            $arShoppingCart[$key]['PRICE'] = $_POST["PRODUCT"][$item['BASKET_ID']]['PRICE'];
            $arShoppingCart[$key]['PRICE_DEFAULT'] = $_POST["PRODUCT"][$item['BASKET_ID']]['DISCOUNT_PRICE'] + $_POST["PRODUCT"][$item['BASKET_ID']]['PRICE'];
        }

        ob_start();
        var_dump($arShoppingCart);
        $output = ob_get_clean();
        file_put_contents('test.txt', $output);
        ob_start();
        var_dump($arOrderPropsValues);
        $output = ob_get_clean();
        file_put_contents('test4.txt', $output);
	...
	*/
	if (isset($product) && isset($user_id))
	{
        ...
        foreach ($arShoppingCart as $key => $val) {
            $arShoppingCart[$key]['PRICE'] = $val['PRICE_DEFAULT'] / (1 + $val['DISCOUNT_PRICE_PERCENT'] / 100);
        }
...
//            echo "<pre>";
                    ...
                    //$val['PRICE'] = $val['PRICE_DEFAULT'] / (1 + $val['DISCOUNT_PRICE_PERCENT']/100);
//                    $discount = 0;
//                    foreach ($product as $key => $value)
//                    {
//                        if ($key == $val["TABLE_ROW_ID"]) $discount = $value['DISCOUNT_PRICE'];
//                    }
//                    $val['PRICE'] += $discount;
					$priceBase = $val["PRICE"] + $val['PRICE_DEFAULT'] * $val['DISCOUNT_PRICE_PERCENT'] / 100;//$val["DISCOUNT_PRICE"];
					$priceDiscountPercent = roundEx(($val["DISCOUNT_PRICE"] * 100) / $priceBase, SALE_VALUE_PRECISION);



//                    $dscnt = $val['PRICE_BASE'] - $val['PRICE_BASE'] / (1 + floatval(str_replace('%', '', $val["DISCOUNT_PRICE_PERCENT"])) / 100);
                    $dscnt = $val['PRICE_DEFAULT'] - $val['PRICE'];

                    $dbBasket = CSaleBasket::GetList(Array(), Array('PRODUCT_ID' => $val['PRODUCT_ID'], 'ORDER_ID' => $id))->Fetch();
//                    while ($arBasket = $dbBasket->Fetch())
//                    {
//                        $arR[] = $arBasket;
//                    }
                    $arBasketInfo = Array(/*'PRICE'=> $val['PRICE'] + $dscnt,*/ 'DISCOUNT_PRICE' => $dscnt);
                    CSaleBasket::Update($dbBasket['ID'], $arBasketInfo);
                    unset($arBasketInfo);
//                    ob_start();
//                    var_dump($dbBasket);
//                    $output = ob_get_clean();
//                    file_put_contents('test3.txt', $output);

					$arData[$val["TABLE_ROW_ID"]]["PRICE_BASE"] = CCurrencyLang::CurrencyFormat($val["PRICE_DEFAULT"], $val["CURRENCY"], false);
					$arData[$val["TABLE_ROW_ID"]]["DISCOUNT_REPCENT"] = $val["DISCOUNT_VALUE"];//$priceDiscountPercent;
					$arData[$val["TABLE_ROW_ID"]]["DISCOUNT_PRICE"] = $dscnt;//$val["DISCOUNT_PRICE"];
					$arData[$val["TABLE_ROW_ID"]]["PRICE"] = $val["PRICE"] + $dscnt;
					...
					$arData[$val["TABLE_ROW_ID"]]["SUMMA_DISPLAY"] = CCurrencyLang::CurrencyFormat((($val["PRICE_DEFAULT"] - $dscnt)     * $val["QUANTITY"]), $val["CURRENCY"], false);
...
//					$priceBaseTotal += ($priceBase * $val["QUANTITY"]);
					$priceBaseTotal += ($val["PRICE_DEFAULT"] * $val["QUANTITY"]);
                    $totalDiscount += $dscnt;
				}
			}
...
//			$arData[0]["DELIVERY_PRICE"] = $arOrder["PRICE_DELIVERY"];
			$arData[0]["DELIVERY_PRICE"] = $_POST['deliveryPrice'];
//			$arData[0]["DELIVERY_PRICE_FORMAT"] = SaleFormatCurrency($arOrder["PRICE_DELIVERY"], $currency);
			$arData[0]["DELIVERY_PRICE_FORMAT"] = SaleFormatCurrency($_POST['deliveryPrice'], $currency);
		}
		else
		{
...
//			$arData[0]["DELIVERY_PRICE_FORMAT"] = SaleFormatCurrency($arDelivery["DELIVERY_DEFAULT_PRICE"], $currency);
        ...
        //$arOrder["PRICE"] = $arOrder["PRICE"] + $_POST['deliveryPrice'];
        ...
        $arData[0]["PRICE_WITH_DISCOUNT_FORMAT"] = SaleFormatCurrency($priceBaseTotal - $totalDiscount , $currency);
//		$arData[0]["PRICE_WITH_DISCOUNT"] = roundEx($arOrder["ORDER_PRICE"]);
		$arData[0]["PRICE_WITH_DISCOUNT"] = roundEx($priceBaseTotal - $totalDiscount);
...
//		$arData[0]["PRICE_TO_PAY"] = SaleFormatCurrency($arOrder["PRICE"] + $_POST['deliveryPrice'] , $currency);
		$arData[0]["PRICE_TO_PAY"] = SaleFormatCurrency($priceBaseTotal + $_POST['deliveryPrice'] - $totalDiscount , $currency);
//		$arData[0]["PRICE_TO_PAY_DEFAULT"] = floatval($arOrder["PRICE"] + $_POST['deliveryPrice']);
		$arData[0]["PRICE_TO_PAY_DEFAULT"] = floatval($arOrder["ORDER_PRICE"] + $_POST['deliveryPrice'] - $totalDiscount);
...
//            $discountValue = str_replace('%', '', $val['DISCOUNT_VALUE']);
//            $val['DISCOUNT_PRICE'] = $val['PRICE'] * $discountValue / 100;
			...
			$productCountAll += $val["QUANTITY"];
                ...
                $dbProps = CSaleOrderPropsValue::GetOrderProps($_GET['ID']);

                while($arProps = $dbProps->Fetch())
                {
                    if ($arProps['CODE'] == 'DELIVERY_COUNTRY') $deliveryCountryCode = $arProps['VALUE'];
                }
            ...
            $basePrice = 0;
                ...
                $dbBasket = CSaleBasket::GetList(Array(), Array('ORDER_ID' => $_GET['ID']), false, false, Array('ID'))->Fetch();
               $dbBasketProps = CSaleBasket::GetPropsList(Array(), Array("BASKET_ID" => $dbBasket['ID']));

                while ($arBasketProps = $dbBasketProps->Fetch())
                {
                    if ($arBasketProps['CODE'] == 'BASE_PRICE_P') $basePrice = $arBasketProps['VALUE'];
                }

                $SKU = CCatalogSku::GetProductInfo($val['PRODUCT_ID']);
...
//                $val['VAT_RATE'] = ($group == 554) ? 1.12 : 1.21;
//                $val["PRICE"] = number_format(($useVAT) ? $val["PRICE"] : $val["PRICE"] / $val["VAT_RATE"], 2, '.', '');


			if (!CSaleBasketHelper::isSetItem($val))
			{
//                echo $val["PRICE"] . "  " . $val["DISCOUNT_PRICE"] . "  ";
				$ORDER_TOTAL_PRICE += ($val["PRICE"] + $val["DISCOUNT_PRICE"]) * $val["QUANTITY"];
				$ORDER_PRICE_WITH_DISCOUNT += ($val["PRICE"] - $val["DISCOUNT_PRICE"]) * $val["QUANTITY"];
			}
			...
			foreach ($arUserColumns as $columnCode => $columnName)
			{
				...
				if ($columnCode == "COLUMN_PRICE")
				{
					...
					<td class="COLUMN_PRICE" style="white-space: nowrap;">
						...
						<div id="DIV_PRICE_OLD_<?=$val["ID"]?>" class="base_price" style="display:none;"><?=CCurrencyLang::CurrencyFormat($val["PRICE"], $str_CURRENCY, false);?> <span><?=$CURRENCY_FORMAT?></span></div>

						<?
						if ($priceDiscount > 0)
//							$discountPercent = "(".GetMessage('NEWO_PRICE_DISCOUNT')." ".$priceDiscount."%)";
							$discountPercent = "(".GetMessage('NEWO_PRICE_DISCOUNT')." ".$val['DISCOUNT_VALUE'].")";
                ...
                if ($columnCode == "COLUMN_BASE_PRICE")
                {
                    ...
                    foreach ($val['PROPS'] as $item)
                    {
                        if ($item['CODE'] == 'BASE_PRICE_P') $basePrice = $item['VALUE'];
                    }
                    ...
                    <td class="COLUMN_BASE_PRICE" nowrap style="in-width: 100px !important; text-align: right !important; vertical-align: top;">
                        <div class="edit_price">
													<span class="default_price_product" >
														<span class="formated_price"><?=CCurrencyLang::CurrencyFormat($basePrice, $val["CURRENCY"], false);?></span>
													</span>
...
//			prOld = parseFloat(BX('PRODUCT[' + i + '][PRICE_DEFAULT]').value);
            ...
            // let price = parseFloat(discount) + parseFloat(BX('PRODUCT[' + i + '][PRICE]').value);
            // let priceDefault = parseFloat(discount) + parseFloat(BX('PRODUCT[' + i + '][PRICE_DEFAULT]').value);
	...
	function fRecalProductResult(result)
	{
		...
		if (result.length > 0)
		{
			...
			for (i in res)
			{
				if (i > 0)
				{
					BX('PRODUCT[' + i + '][PRICE]').value = res[i]["PRICE_DISPLAY"];
