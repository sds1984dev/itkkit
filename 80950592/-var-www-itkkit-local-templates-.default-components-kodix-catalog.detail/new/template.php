                                ...
                                <div class="price__wrapper price__wrapper--product">
                                    <div class="price">
                                        <span class="price__block">
                                            <span class="price--current"><?printf("%3.2f",KDXCurrency::convert($useVAT=="N" ? $arOfferPrice['PRICE'] / $arSKU['VAT_RATE'] : $arOfferPrice['PRICE'], $curCurrency));?></span>
                        ...
                        <div data-product-price="<?=$arSKU['ID']?>" style="display: none">
                            <div class="price__wrapper price__wrapper--product">
                                <div class="price <?if($arSKU['BASE_PRICE'] > $arSKU['RETAIL_PRICE']){?>price--has-discount"<?}?>>
                                    <span class="price__block">
                                        <?$curPrice = KDXCurrency::convert($useVAT=="N" ? $arSKU['RETAIL_PRICE'] / $arSKU['VAT_RATE'] : $arSKU['RETAIL_PRICE'], KDXCurrency::$CurrentCurrency);?>
                                        <span class="price--current"><?printf("%3.2f",KDXCurrency::convert($useVAT=="N" ? $arSKU['RETAIL_PRICE'] / $arSKU['VAT_RATE'] : $arSKU['RETAIL_PRICE'], $curCurrency))?></span>
                                        <span class="price--currency"><?=KDXCurrency::GetCurrencyName($curCurrency)?></span>
                                    </span>
                            ...
                            <?$approx_rub_price = $arSKU['RETAIL_PRICE'] / $arSKU['VAT_RATE'];?>
                            <div class="txt--grey">
                        ...
                        <div class="price__wrapper price__wrapper--product">
                            <div class="price <?if($arResult['BASE_PRICE_MIN'] > $arResult['RETAIL_PRICE_MIN']){?>price--has-discount<?}?>">
                                ...
                                <?if($arResult['BASE_PRICE_MIN'] > $arResult['RETAIL_PRICE_MIN']){?>
                                    <span class="price--old">
                                        <span><?printf("%3.2f",KDXCurrency::convert($useVAT=="N" ? $arSKU['BASE_PRICE'] / $arSKU['VAT_RATE'] : $arSKU['BASE_PRICE'], $curCurrency))?></span>
                                        <span class="price--currency"><?=KDXCurrency::GetCurrencyName($curCurrency)?></span>
                                    </span>
                                <?}?>
                            </div>
                        </div>
                        <?$approx_rub_price = $arResult['RETAIL_PRICE_MIN'] / $arSKU['VAT_RATE'];?>
...
<script type="application/ld+json">
{
  ...
  "offers": {
    ...
    <?if (LANGUAGE_ID == 'ru'){?>
        "priceCurrency": "RUB",
        <?if ($arResult['HAVE_REAL_SIZES']){?>
            "price": "<?printf("%3.2f",KDXCurrency::convert($useVAT=="N" ? $arSKU['RETAIL_PRICE'] / $arSKU['VAT_RATE'] : $arSKU['RETAIL_PRICE'], 'RUB'))?>",
        <?} else {
            $resOffersList = CCatalogSKU::GetInfoByProductIBlock($arResult['IBLOCK_ID']);
            if (is_array($resOffersList)){
                $rsOffers = CIBlockElement::GetList(array('PRICE'=>'DESC'),array('IBLOCK_ID'=>$resOffersList['IBLOCK_ID'], 'PROPERTY_'.$resOffersList['SKU_PROPERTY_ID']=>$arResult["ID"]));
                if ($arOffer = $rsOffers->GetNext()){
                    $arOfferPrice = GetCatalogProductPrice($arOffer["ID"], 1);?>
                    "price": "<?printf("%3.2f",KDXCurrency::convert($useVAT=="N" ? $arOfferPrice['PRICE'] / $arSKU['VAT_RATE'] : $arOfferPrice['PRICE'], 'RUB'));?>",
                <?}
            }
        }
    } else {?>
        "priceCurrency": "<?=KDXCurrency::$CurrentCurrency?>",
        "price": "<?printf("%3.2f",KDXCurrency::convert($useVAT=="N" ? $arSKU['RETAIL_PRICE'] / $arSKU['VAT_RATE'] : $arSKU['RETAIL_PRICE'], KDXCurrency::$CurrentCurrency))?>",
    <?}?>
