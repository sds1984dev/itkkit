...
?>
<?
//$temp = \CSaleLocation::getCountryByID($arResult['ORDER']->properties['DELIVERY_COUNTRY']['VALUE']);
$vat_price = $arResult['ORDER']->vat_price;

use Bitrix\Highloadblock as HL;
if ($vat_price==false){
    //Подсчет $vat_price через запрос к HL блоку со странами
    ...
    if ($is_vat) {
        $vat_price=$arResult['ORDER']->properties['VAT_PRICE']['VALUE'];
    }
}
...
<!--                            <div class="cart__article">Артикул: <span>49JEHDN93U</span></div>-->
                    ...
                    <?
                    $price_to_print = $arItem['PRICE'];
                    if ($arResult['ORDER']->add_vat == false)
                    {
                        //$vat = (in_array($arItem['SECTION_ID'], $arResult['arVAT12'])) ? 1.12 : 1.21;
                        $price_to_print = $arItem['PRICE'] / ($arItem['VAT_RATE'] + 1);
                    }
                    $sum_order += $price_to_print;
                    ?>
                    <div class="cart__price">
                        <div class="price__wrapper">
                            <div class="price <?if($arItem['DISCOUNT_PRICE'] > 0){?>price--has-discount<?}?>">
                                <span class="price__block">
                                    <span class="price--current"><?=KDXCurrency::convertAndFormat($price_to_print,KDXCurrency::$CurrentCurrency,$arItem['CURRENCY'])?></span>
                                </span>
...
$total_price_with_discount = $sum_order - $arResult['ORDER']->discount_value;
$total_discount = $sum_order-$total_price_with_discount;
?>
                ...
                <tr>
                    ...
                    <td>
                        <div class="price__wrapper">
                            <div class="price">
                                <span class="price__block">
                                    <?=KDXCurrency::convertAndFormat($sum_order,KDXCurrency::$CurrentCurrency,$arResult['ORDER']->currency)?>
                                </span>
                            </div>
                        </div>
                    </td>
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="price__wrapper">
                                <div class="price">
                                <span class="price__block">
                                    <?=KDXCurrency::convertAndFormat($sum_order - $vat_price,KDXCurrency::$CurrentCurrency,$arResult['ORDER']->currency);?>
                                </span>
                                </div>
                            </div>
                        </td>
