...
if (LANGUAGE_ID == 'en'){
    $APPLICATION->SetPageProperty('title', 'Shop '.$arResult['VARIABLES']['COLOR'].' '.$arCurrentSection.' '.$arCurrentBrand.' at itk online store');
    $APPLICATION->SetPageProperty('description', 'Shop '.$arResult['VARIABLES']['COLOR'].' '.$arCurrentSection.' '.$arCurrentBrand.' at itk online store. Free Shipping on All Orders Over &euro;350 &#10003; Fast Delivery &#10003; 14 Days Return Policy &#10003; 100% Authenticity &#10003;.');
} else {
    $APPLICATION->SetPageProperty('title', 'Купить '.$arResult['VARIABLES']['COLOR'].' '.$arCurrentSection.' '.$arCurrentBrand.' в онлайн магазине itk');
    $APPLICATION->SetPageProperty('description', 'Купить '.$arResult['VARIABLES']['COLOR'].' '.$arCurrentSection.' '.$arCurrentBrand.' в онлайн магазине itk. Бесплатная доставка на все заказы свыше €350 &#10003; Быстрая доставка &#10003; 14 дневная политика возврата товаров &#10003; 100% оригинальный товар &#10003;');   
}
...
$APPLICATION->SetTitle(ucfirst($arResult['VARIABLES']['COLOR']).' '.$arCurrentSection.' '.$arCurrentBrand);
...
$parentDir = str_replace('/brand/'.$arResult['VARIABLES']['BRAND'].'/'.$arResult['VARIABLES']['COLOR'], '', explode('?', $_SERVER['REQUEST_URI'], 2)[0]);
$color = '';
$tagTitle = '';
$arrFilter = [];
if (!in_array(strtolower($arResult['VARIABLES']['COLOR']), getAllColors())){
    $resTag = CIBlockElement::GetList(array(),array('IBLOCK_ID'=>21,'=IBLOCK_SECTION_ID'=>585,'CODE'=>$arResult['VARIABLES']['COLOR'],'=PROPERTY_SHOW'=>$parentDir),false,array(),array('*','PROPERTY_*'));
    if ($arTag = $resTag->GetNextElement()){
        $arTagFields = $arTag->GetFields();
        $arTagProps = $arTag->GetProperties();
        $tagTitle = $arTagFields['NAME'];
        $APPLICATION->SetPageProperty('title', 'Shop '.$brandName.' '.$arTagFields['NAME'].' '.$arCurrentSection.' at itk online store');
        $APPLICATION->SetPageProperty('description', 'Shop '.$brandName.' '.$arTagFields['NAME'].' '.$arCurrentSection.' at itk online store. Free Shipping on All Orders Over €350 ✓ Fast Delivery ✓ 14 Days Return Policy ✓ 100% Authenticity ✓.');
...
if(!isAjax()){
    if (LANGUAGE_ID == 'en'){
        if (!empty($color)){
            getTagPageDescription($arParams['CATALOG_IBLOCK_ID'], $arResult['VARIABLES']['SECTION_ID'], $brand, $arCurrentBrand, strtolower($arCurrentSection), $color);
        }
    }
}
