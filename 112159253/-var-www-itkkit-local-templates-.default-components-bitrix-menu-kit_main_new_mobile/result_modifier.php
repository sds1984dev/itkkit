...
while ($arElement = $dbElements->Fetch()) {
    ...
    $arBrand = CIBlockElement::GetList(array(), array('IBLOCK_ID' => 3, 'ID' => $arProperty[1][0]), false, false, array('ID', 'CODE', 'NAME', 'PROPERTY_SHOW_MENU_SALE'))->Fetch();
 
    if($arBrand['PROPERTY_SHOW_MENU_SALE_VALUE'] == 'Y') {
        if (!in_array($arBrand['NAME'], $arBrandExist)) {
            $formattedPath = '/catalog/brand/' . $arBrand['CODE'] . '/sale/';
...
usort($arMenuSaleBrands, 'sort_subitems');
usort($arMenuSale, 'sort_subitems');
