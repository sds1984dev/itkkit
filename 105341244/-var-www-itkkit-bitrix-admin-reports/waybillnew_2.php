...
if (isset($hlblock['ID']))
{
    ...
    {
        if ((isset($item['UF_USE_VAT'])) and ($item['UF_USE_VAT']=='Y')) {
            $vat = 1.21;
//            $arVAT12 = array(554);
...
foreach ($products as $key=>$item) {
    ...
    if ($item['DISCOUNT_COUPON']!=='') $DISCOUNT_COUPON=$item['DISCOUNT_COUPON'];
}
            ...
            <div class='table_row'>
                <table class='report_table'>
                    ...
                    while ($arBasket = $dbOrderBasket->Fetch()) {
                        $arDiscounts[$arBasket["ID"]] = $arBasket["DISCOUNT_PRICE"] / 1.21;
                    }

                    foreach ($products as $item) {
                        //добавить проверку на страну
                        $p_val['quantity']=number_format($item['QUANTITY']);
                        $p_val['unit_price-vat']=round($item['PRICE']/1.21, 2);
                        //$p_val['unit_price-vat']=round($item['PRICE']/($item['VAT_RATE']+1), 2);
                        $p_val['vat']=number_format($item['PRICE']-$item['PRICE']/$vat, 2, '.', '');
                        //$p_val['vat']=number_format($item['PRICE']-$item['PRICE']/($item['VAT_RATE']+1), 2, '.', '');
                        ...
                        //$sum += floatval(number_format($item['PRICE'] + $item['PRICE']*1.21, 2, '.', ''));
                        ...
                        <tr>
                            ...
                            if ($discount) {
                                $discount_value = $arDiscounts[$item["ID"]] * $item["QUANTITY"]; ?>
                                <td><?=number_format($discount_value, 2)?> EUR</td>
                            <?
                                }?>
                            <?//if ($print_discount) {
//                                //$discount_value=$item['BASE_PRICE']-$item['PRICE'];
//
//                                if ($p_val['vat']==0) {$discount_value=round($discount_value/1.21,2);}
//                                ?><!--<td>--><?//=$discount_value?><!-- EUR</td>--><?//}?>
                        </tr>
                        <?
                        ?>
                    <? } ?>    
                ...
                <div class='discount_row'>
                    <?if ($DISCOUNT_COUPON!=='None') {?>
                        <span class='summary_name'>COUPON: <?=$DISCOUNT_COUPON?></span>
                    <?}?>
                </div>     
...
<!--                <span class='summary_value'><?=$sum_without_vat?> EUR</span>-->
                <?//проблема 1 цента?>
                <span class='summary_value'><?=$arOrder['PRICE']-$sum_vat-number_format($arOrder['PRICE_DELIVERY'], 2, '.', '')?> EUR</span>
                </br>
                ...
                <span class='summary_value summary_total'><?=$arOrder['PRICE']?> EUR</span>
                </br>
            </div>  
